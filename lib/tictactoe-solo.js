...
module.exports = async (lexxy, chats, prefix, ttcsolo, msg) => {
...
Ketik ${prefix}endttc untuk berhenti bermain`
            lexxy.sendMessage(from, { text: tekss, mentions: [posi.penantang] }, { messageId: 'TTCS'+makeid(12).toUpperCase() })
            ...
            lexxy.sendMessage(from, { text: teksr }, { messageId: 'TTCS'+makeid(12).toUpperCase() })
        ...
        if (posi.giliran == 'bot' ? posi.ditantang == sender : true) {
          for (let i of nomor) {
            if (Number(chats) === i) {
              if (cekIsi(Number(chats) - 1, anu)) return lexxy.sendMessage(from, { text: `Nomor tersebut sudah terisi` }, { quoted: msg, messageId: 'TTCS'+makeid(12).toUpperCase() })
...
Ingin bermain lagi? ${prefix}ttc-solo`
                lexxy.sendMessage(from, { text: teksw, mentions: [posi.penantang] }, { messageId: 'TTCS'+makeid(12).toUpperCase() })
                ttcsolo.splice(getPosTic(from, ttcsolo), 1)
...
Ingin bermain lagi? ${prefix}ttc-solo`
                lexxy.sendMessage(from, { text: teksi, mentions: [posi.penantang] }, { messageId: 'TTCS'+makeid(12).toUpperCase() })
                ttcsolo.splice(getPosTic(from, ttcsolo), 1)
              } else {
...
Ketik ${prefix}endttc untuk berhenti bermain`
                lexxy.sendMessage(from, { text: teksg, mentions: [posi.penantang] }, { messageId: 'TTCS'+makeid(12).toUpperCase() })
